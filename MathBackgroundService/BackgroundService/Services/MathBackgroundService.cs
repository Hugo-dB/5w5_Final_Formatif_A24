    ...
    private async Task EvaluateChoices()
    {
        ...
        foreach (var userId in _data.Keys)
        {
            using (IServiceScope scope = _serviceScopeFactory.CreateScope())
            {
                ...
                {
                    await _mathQuestionHub.Clients.User(userId).SendAsync("Answer", "Bonne réponse !");

                    Player player = await dbContext.Player.Where(p => p.UserId == userId).FirstOrDefaultAsync();
                    player.NbRightAnswers++;         
                }
                else
                {
                    await _mathQuestionHub.Clients.User(userId).SendAsync("Answer", "Mauvaise réponse ! La bonne réponse était " + _currentQuestion.Answers[_currentQuestion.RightAnswerIndex]);
                }
