    ...
    private async Task EvaluateChoices()
    {
        using (IServiceScope scope = _serviceScopeFactory.CreateScope())
        {
            ...
            foreach (var userId in _data.Keys)
            {
                ...
                {
                    Player player = await dbContext.Player.Where(p => p.UserId == userId).SingleAsync();
                    player.NbRightAnswers++;
                    await dbContext.SaveChangesAsync();

                    await _mathQuestionHub.Clients.User(userId).SendAsync("Result", "Bonne réponse !");
                    await _mathQuestionHub.Clients.User(userId).SendAsync("PlayerInfo", new PlayerInfoDTO() { NbRightAnswers = player.NbRightAnswers });
                }
                else
                {
                    await _mathQuestionHub.Clients.User(userId).SendAsync("Result", "Mauvaise réponse ! La Bonne réponse était " + _currentQuestion.Answers[_currentQuestion.RightAnswerIndex]);
                }
        }
